...
ASSERT(FALSE);
    ...
    Status = PpIrpQueryCapabilities(DeviceNode, &DeviceCapabilities);
    if (!NT_SUCCESS(Status))
    {
    ...
    _Out_ PUNICODE_STRING InstancePath)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("PpIrpQueryCapabilities() failed (Status 0x%08lx)\n", Status);
                                ...
                                PVOID Context)
{
    ...
    if (!(DeviceNode->Flags & (DNF_DISABLED | DNF_STARTED | DNF_ADDED)))
    {
        ...
        {
            if (NT_SUCCESS(PpIrpQueryCapabilities(DeviceNode, &DeviceCaps)) &&
...
IoRequestDeviceEject(IN PDEVICE_OBJECT PhysicalDeviceObject)
{
    ...
    if (PpIrpQueryCapabilities(DeviceNode, &Capabilities) != STATUS_SUCCESS)
    {
        goto cleanup;
    }
