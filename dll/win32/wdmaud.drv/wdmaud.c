    ...
    LONG Parameter2)
{
    switch ( Message )
    {
        case DRV_LOAD :
        {
...
#ifndef USE_MMIXER_LIB
            DeviceInfo = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(WDMAUD_DEVICE_INFO) + sizeof(L"BogusDeviceString"));
            if (!DeviceInfo)
            {
                /* No memory */
            ...
            lstrcpyW(DeviceInfo->DeviceInterfaceString, L"BogusDeviceString");
