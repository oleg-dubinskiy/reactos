    ...
    IN  PWAVEHDR Header)
{
    ...
    /* Start audio device if needed */
    if (!DeviceInfo->DeviceState->bStart)
    {
        Result = FUNC_NAME(WdmAudSetWaveState)(DeviceInfo, TRUE);
        if (!MMSUCCESS(Result))
        {
            Result = FUNC_NAME(WdmAudResetStream)(DeviceInfo, TRUE);
            if (!MMSUCCESS(Result))
            {
                DPRINT1("WdmAudResetStream failed with error %d\n", GetLastError());
                return TranslateInternalMmResult(Result);
            }
        }
    }
