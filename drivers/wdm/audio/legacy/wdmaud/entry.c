    ...
    IN PVOID Context)
{
    ...
    {
        /* init mmixer library */
        Status = WdmAudMixerInitialize(FileObject);
        ...
        for (DeviceIndex = 0; DeviceIndex < DeviceExtension->SysAudioDeviceCount; DeviceIndex++)
        {
            Status = SetSysAudioDeviceInstance(FileObject, DeviceIndex);
            if (!NT_SUCCESS(Status))
            {
                /* failed, move to the next instance */
                DPRINT1("Failed to set device index, skipping...\n");
                continue;
            }
        }
    }
    ...
    IN  PIRP Irp)
{
    ...
    Status = KsReferenceSoftwareBusObject(DeviceExtension->DeviceHeader);
    if (!NT_SUCCESS(Status))
    {
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        KsDereferenceSoftwareBusObject(DeviceExtension->DeviceHeader);
        return SetIrpIoStatus(Irp, Status, 0);
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        KsDereferenceSoftwareBusObject(DeviceExtension->DeviceHeader);
        IoUnregisterPlugPlayNotification(DeviceExtension->SysAudioNotification);
        DeviceExtension->SysAudioNotification = NULL;
        /* complete and forget */
        return SetIrpIoStatus(Irp, Status, 0);
    }
